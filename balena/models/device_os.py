...
class DeviceOs(object):
    ...
    def download_unconfigured_image(self, device_type, version='latest', raw=False):
        """
        Download an unconfigured OS image.

        Args:
            device_type (str): device type slug.
            version (str): the Balena OS version of the image (defaults to 'latest'). The SDK will try to parse version into semver-compatible version, unsupported (unpublished) version will result in rejection.
            raw (bool): determining function return value.
